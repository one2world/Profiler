    ...
    internal class CachedSnapshot : IDisposable
    {
        ...
        public unsafe class SceneRootEntriesCache : IDisposable
        {
            ...
            public SceneRootEntriesCache(ref IFileReader reader)
            {
                ...
                using (var tmp = new DynamicArray<byte>(0, Allocator.TempJob, true))
                {
        ...
        public struct ManagedMemorySectionEntriesCache : IDisposable
        {
                ...
                ref NestedDynamicSizedArrayReadOperation<byte> associatedByteArrayReadOp, bool isStackMemory)
            {
                ...
                unsafe
                {
                    ...
                    // 修复: elementSize 应该是 sizeof(long)，而不是 sizeof(SortIndexHelper)
                    // 因为我们只想复制 Index 字段（long），而不是整个结构体
                    UnsafeUtility.MemCpyStride(sortedIndice.GetUnsafePtr(), sizeof(long), sortMapping.GetUnsafePtr(), sizeof(SortIndexHelper), sizeof(long), (int)startAddresses.Count);
